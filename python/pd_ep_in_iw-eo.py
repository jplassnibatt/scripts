...
class PagerDutyAPI:
    ...
    def request(self, url: str, params: Optional[Dict] = None, max_retries: int = 3) -> Optional[requests.Response]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code in (401, 403):
                    logger.error(f"Authentication failed (HTTP {response.status_code}). Check PAGERDUTY_API_TOKEN.")
                    sys.exit(1)
...
class OrchestrationAnalyzer:
        ...
        def search_rules(obj, current_rule_id=None):
            if isinstance(obj, dict):
                if 'id' in obj and 'actions' in obj:
                    ...
                    if ep_id in json.dumps(obj) and current_rule_id not in rule_ids:
    ...
    def get_escalation_policies(self) -> Dict[str, str]:
        ...
        policies = self.api.paginated_get('/escalation_policies', 'escalation_policies')
    ...
    def get_services(self) -> List[Dict[str, str]]:
        ...
        for service in services_data:
            if 'html_url' in service:
                ...
                if self.subdomain:
                    logger.info(f"✓ Auto-detected PagerDuty subdomain: {self.subdomain}")
                    break
    ...
    def process_workflows(self, ep_map: Dict[str, str]) -> List[dict]:
        ...
        workflows_data = self.api.paginated_get('/incident_workflows', 'incident_workflows')
        ep_ids_set = set(ep_map.keys())
        matches = []
        
        for wf in workflows_data:
            response = self.api.request(f"{self.api.base_url}/incident_workflows/{wf['id']}")
            if not response:
                continue
            ...
            for ep_id in found_eps:
                url = f"https://{self.subdomain}.pagerduty.com/incident-workflows/workflows/{wf['id']}" if self.subdomain else ''
                matches.append({
    ...
    def process_global_orchestrations(self, ep_map: Dict[str, str]) -> List[dict]:
        logger.info("Analyzing Global Event Orchestrations...")
        orchestrations = self.api.paginated_get('/event_orchestrations', 'orchestrations')
        ep_ids_set = set(ep_map.keys())
        matches = []
        
        for orch in orchestrations:
            response = self.api.request(f"{self.api.base_url}/event_orchestrations/{orch['id']}/global")
            if not response:
                continue
            ...
            for ep_id in found_eps:
                ...
                for rule_id in rule_ids or [None]:
                    url = f"https://{self.subdomain}.pagerduty.com/event-orchestration/{orch['id']}/global-rules/rule/{rule_id}" if self.subdomain and rule_id else ''
                    matches.append({
                        'incident_workflow_id': '',
                        'incident_workflow_name': '',
                        'event_orchestration_name': orch.get('name', orch['id']),
                        'service_name': '',
                        'escalation_policy_id': ep_id,
                        'escalation_policy_name': ep_map[ep_id],
                        'url': url
    ...
    def process_service_orchestrations(self, services: List[Dict[str, str]], ep_map: Dict[str, str]) -> List[dict]:
        logger.info("Analyzing Service Event Orchestrations...")
        ep_ids_set = set(ep_map.keys())
        matches = []
        
        for svc in services:
            response = self.api.request(f"{self.api.base_url}/event_orchestrations/services/{svc['id']}")
            if not response:
                continue
            ...
            for ep_id in found_eps:
                rule_ids = self.extract_rule_ids(details, ep_id)
                for rule_id in rule_ids or [None]:
                    url = f"https://{self.subdomain}.pagerduty.com/event-orchestration/service/{svc['id']}/rule/{rule_id}" if self.subdomain and rule_id else ''
                    matches.append({
                        'incident_workflow_id': '',
                        'incident_workflow_name': '',
                        'event_orchestration_name': '',
...
def export_to_csv(
    ...
    fieldnames = [
        'incident_workflow_id', 'incident_workflow_name', 'event_orchestration_name',
        'service_name', 'escalation_policy_id', 'escalation_policy_name', 'url'
    ]
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-o", "--output", default="pagerduty_ep_dependencies", help="Custom CSV filename prefix"
    )
    return parser
...
def main() -> None:
    ...
    if not api_token or api_token.strip() == "YOUR_API_TOKEN_HERE":
        logger.error("ERROR: PAGERDUTY_API_TOKEN environment variable is missing or invalid.")
        logger.error("Set it using: export PAGERDUTY_API_TOKEN='your-token-here'")
        sys.exit(1)
    ...
    try:
        ...
        matches.extend(analyzer.process_service_orchestrations(services, ep_map))
